...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  override def mostRetweeted: Tweet = {
    ...
    val bigger =
      if (leftBiggest.retweets > rightBiggest.retweets) leftBiggest
      else rightBiggest
  ...
  def descendingByRetweet(space: TweetSet, acc: TweetList): TweetList = {
    if (space.isEmpty) acc
    else {
      val biggest = space.mostRetweeted
      val newAcc = descendingByRetweet(space.remove(biggest), acc)
...
object GoogleVsApple {
  ...
  def containsKeywords(tw: Tweet, keywords: List[String]): Boolean = {
    if (keywords.isEmpty) false
      ...
      if (tw.text.contains(keywords.head)) true
      else containsKeywords(tw, keywords.tail)
    }
  }

  def searchTweets(allTweets: TweetSet, keywords: List[String]): TweetSet = {
  ...
  lazy val googleTweets: TweetSet = searchTweets(TweetReader.allTweets, apple)
  lazy val appleTweets: TweetSet = searchTweets(TweetReader.allTweets, apple)
...
object Main extends App {
  ...
  GoogleVsApple.trending foreach {
    println _
  }
}
